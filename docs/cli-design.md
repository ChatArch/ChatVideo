# ChatVideo CLI Design

This document records the first provider-neutral CLI shape for ChatVideo. It is based on practical video workflows, but it intentionally avoids private project details, source media, service URLs, task ids, account names, and credentials.
...
- Records duration, codec, audio presence, and output size.
...
- Writes job manifests with provider names and safe status fields only.
...
- Splits multi-frame stories into adjacent segment jobs.
...
This PR adds the `chatvideo design` command and documentation only. It does not implement provider adapters, submit network jobs, or publish files. Future PRs can implement each command group behind the documented privacy contract.
